...
@app.function_name(name="query_transcripts")
@app.route(route="query_transcripts", methods=["GET"])
def query_transcripts(req: func.HttpRequest) -> func.HttpResponse:
    logging.info('HTTP trigger function to query transcripts.')
    ...
    try:
        ...
        min_score = req.params.get('min_score')
        score_type = req.params.get('score_type')  # politeness, empathy, or professionalism

        # Connect to Cosmos DB
        cosmos_client = CosmosClient(os.getenv("COSMOS_ENDPOINT"), os.getenv("COSMOS_KEY"))
        database = cosmos_client.get_database_client(os.getenv("COSMOS_DATABASE"))
        transcript_container = database.get_container_client(os.getenv("COSMOS_CONTAINER1"))
        analysis_container = database.get_container_client(os.getenv("COSMOS_CONTAINER2"))
        ...
        if search_text:
            query += " AND CONTAINS(c.transcript, @search_text)"
            parameters.append({"name": "@search_text", "value": search_text})

        # Get transcripts
        transcripts = list(transcript_container.query_items(
            query=query,
            parameters=parameters,
            enable_cross_partition_query=True
        ))

        # If score filtering is requested, get analyses and filter
        if min_score and score_type:
            min_score = float(min_score)
            analyses = list(analysis_container.query_items(
                query="SELECT * FROM c WHERE c.type = @type AND c.assessment.score >= @min_score",
                parameters=[
                    {"name": "@type", "value": score_type},
                    {"name": "@min_score", "value": min_score}
                ],
                enable_cross_partition_query=True
            ))
            
            # Get call_ids that meet the score criteria
            valid_call_ids = {a['call_id'] for a in analyses}
            
            # Filter transcripts
            transcripts = [t for t in transcripts if t['call_id'] in valid_call_ids]

        # For each transcript, get its analyses
        for transcript in transcripts:
            analyses = list(analysis_container.query_items(
                query="SELECT * FROM c WHERE c.call_id = @call_id",
                parameters=[{"name": "@call_id", "value": transcript['call_id']}],
                enable_cross_partition_query=True
            ))
            
            # Add analyses to transcript object
            transcript['analyses'] = {
                analysis['type']: analysis['assessment']
        ...
        logging.error(f"Error querying transcripts: {e}")
        return func.HttpResponse(
            "Error querying transcripts.",
            status_code=500
        )